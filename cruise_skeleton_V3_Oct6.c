...
#define EXTRA_PRIO         11
#define OVERLOAD_PRIO     13
...
// Semaphores
OS_EVENT *Ctrl_Sem, *Vehicle_Sem, *Switch_Sem, *Button_Sem, *Watchdog_Sem, *Extra_Sem;

// SW-Timer
#define OS_TMR_CFG_TICKS_PER_SET  100 /* 10 ticks per second, i.e. 100ms per tick*/
OS_TMR *ControlTmr; /*other tasks also use this*/
...
int watchdog_count; //------Should be OK as lab manual------
...
void WatchDog(void* pdata){
    ...
    while(1){
        ...
        if(watchdog_count < 0){
            ...
            led_green = led_green | 0x80;           
        }else{
            led_green = led_green & ~0x80;
        }
        IOWR_ALTERA_AVALON_PIO_DATA(DE2_PIO_GREENLED9_BASE, led_green);
        //OSTimeDlyHMSM(0,0,0, WATCHDOG_PERIOD);
    }
}

void OverLoad_detect(void* pdata){
    printf("Overload dectect task created!\n");
    while(1){
        //printf("Overload dectect running!\n");
        watchdog_count = 5;
        //OSTimeDlyHMSM(0,0,0, OVERLOAD_PERIOD);
    }
}

void Extra_load(void* pdata){
    INT8U err;   
    int switch_in,i;
    int x=0;
    printf("Extra load task created!\n");
    while(1){
        OSSemPend(Extra_Sem, 0, &err);
        switch_in = switches_pressed();
        switch_in = switch_in >>4;
        printf("switch_in is %d", switch_in);
        for(i=0;i < 20000*switch_in;i++){
            x=x+i;
        }                  
        // OSTimeDlyHMSM(0,0,0, EXTRA_PERIOD);
    }
}
/*
...
void StartTask(void* pdata)
{
  ...
  Mbox_Velocity = OSMboxCreate((void*) 0); /* Empty Mailbox - Velocity */
   ...
   Switch_Sem = OSSemCreate(1);
   Button_Sem = OSSemCreate(1);
   Watchdog_Sem = OSSemCreate(1);
  ...
  err = OSTaskCreateExt(
   ...
   err = OSTaskCreateExt(
    ...
    err = OSTaskCreateExt(
       OverLoad_detect, // Pointer to task code
       NULL,        // Pointer to argument that is
                    // passed to task
    ...
    err = OSTaskCreateExt(
       Extra_load, // Pointer to task code
       NULL,        // Pointer to argument that is
                    // passed to task
